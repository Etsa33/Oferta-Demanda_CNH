...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- Filtro de categoría del coroplético (columna derecha) ---
  const selCategoria = document.getElementById('sel-categoria-coropletico');
  selCategoria.innerHTML = CONFIG.OPCIONES_COROPLETICO
    .map(o => `<option value="${o.id}">${o.label}</option>`).join('');
  selCategoria.addEventListener('change', (e) => {
    MapModule.actualizarCategoriaCoropletico(e.target.value);
  });
  ...
  const chkModoSeleccion = document.getElementById('btn-dibujar-poligono');
  chkModoSeleccion.addEventListener('click', () => MapModule.iniciarDibujoPoligono());
  ...
  // --- Buscador de Unidades de Atención por código o nombre ---
  const inputBuscarUA = document.getElementById('buscar-ua-texto');
  const contenedorResultadosUA = document.getElementById('resultados-busqueda-ua');

  function renderResultadosBusquedaUA(resultados) {
    contenedorResultadosUA.innerHTML = '';
    if (!resultados.length) {
      contenedorResultadosUA.innerHTML = '<p class="resultado-ua-vacio">Sin coincidencias.</p>';
      return;
    }
    resultados.forEach(r => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'resultado-ua';
      btn.innerHTML = `<strong>${r.codigo}</strong> — ${Utils.tituloCaso(r.nombre)}`;
      btn.addEventListener('click', async () => {
        chkUA.checked = true;
        await MapModule.irAUA(r.marker);
      });
      contenedorResultadosUA.appendChild(btn);
    });
  }

  async function ejecutarBusquedaUA() {
    const texto = inputBuscarUA.value.trim();
    if (!texto) {
      contenedorResultadosUA.innerHTML = '';
      return;
    }
    contenedorResultadosUA.innerHTML = '<p class="resultado-ua-vacio">Buscando…</p>';
    const resultados = await MapModule.buscarUA(texto);
    renderResultadosBusquedaUA(resultados);
    // Si hay una sola coincidencia, ir directo a ella sin necesidad de un clic extra.
    if (resultados.length === 1) {
      chkUA.checked = true;
      MapModule.irAUA(resultados[0].marker);
    }
  }

  document.getElementById('btn-buscar-ua').addEventListener('click', ejecutarBusquedaUA);
  inputBuscarUA.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') ejecutarBusquedaUA();
  });
  ...
  function actualizarTodosLosGraficos(estado) {
    ...
    Graficos.actualizar('provincia', propsProvincia, estado.provincia ? estado.provincia.nombre : null);
    Graficos.actualizar('canton', propsCanton, estado.canton ? estado.canton.nombre : null);
    Graficos.actualizar('parroquia', propsParroquia, estado.parroquia ? estado.parroquia.nombre : null);
  }
