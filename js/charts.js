...
const Graficos = (() => {
  ...
  const NIVEL_TEXTO = {
    ...
    canton: 'Cantón seleccionado',
    parroquia: 'Parroquia seleccionada'
  };
  ...
  function actualizarGrupo(nivel, grupo, props) {
    const els = elementosGrupo(nivel, grupo.id);
    const clave = `${nivel}_${grupo.id}`;
    ...
    els.titulo.textContent = `${grupo.titulo} · ${NIVEL_TEXTO[nivel]}`;
  ...
  function actualizar(nivel, props) {
    CONFIG.GRUPOS_ALERTA.forEach(grupo => actualizarGrupo(nivel, grupo, props));
  }
